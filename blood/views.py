...
def eligibility_check(request):
    if request.method == 'POST':
        ...
        eligible = check_eligibility(age, weight, hemoglobin)
        context = {'eligible': eligible}
        return render(request, 'eligibility_result.html', context)
    else:
...
def blood_facts(request):
   # if request.method=="POST":
        #return redirect("")
    return render(request,"blood_facts.html")
